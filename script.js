...
const sections = {
  ...
  "schedule":    { title: "Schedule",    content: `<p>View the project timeline and milestone schedule here.</p>`, render: function(el, chrome) { if (window.AlignSchedule) window.AlignSchedule.render(el, chrome); } },
  "budget":      { title: "Budget",      content: `<p>Track project costs, change orders, and budget status here.</p>`, render: function(el, chrome) { if (window.AlignBudget) window.AlignBudget.render(el, chrome); } },
  "contacts":    { title: "Directory",   content: ``, render: function(el, chrome) { if (window.AlignContacts) window.AlignContacts.render(el, chrome); } },
  ...
  "tasks":       { title: "Tasks",       content: `<p>Assign, track, and complete project tasks here.</p>`, render: function(el, chrome) { if (window.AlignTasks) window.AlignTasks.render(el, chrome); } },
  "procurement": { title: "Procurement", content: `<p>Manage purchase orders, material orders, and lead times here.</p>`, render: function(el, chrome) { if (window.AlignProcurement) window.AlignProcurement.render(el, chrome); } },
...
function _handleRoute() {
  ...
  var cacheEntry = (sectionKey === 'settings' || sectionKey === 'essentials-config' || sectionKey === 'daily-logs' || sectionKey === 'contacts' || sectionKey === 'project-select' || sectionKey === 'photos' || sectionKey === 'punchlist' || sectionKey === 'drawings' || sectionKey === 'files') ? null : _tileCache[sectionKey];
  ...
  function _renderProjectSelect(el) {
    ...
    fetch('/api/projects', { credentials: 'include' })
      ...
      .then(function(d) {
        ...
        var h = '<div class="ps-page"><div class="ps-header"><div class="ps-logo-wrap"><img class="ps-logo auth-logo-light" src="assets/align-logo-light-v3.png" alt="Align"><img class="ps-logo auth-logo-dark" src="assets/align-logo-dark-v3.png" alt="Align"></div><h1 class="ps-title">Select Project</h1><p class="ps-subtitle">Choose a project to continue</p><div class="ps-header-actions">' + (isAdmin ? '<button class="pm-btn primary ps-new-btn" id="ps-new-project">+ New project</button>' : '') + '<button class="ps-signout-btn" id="ps-signout">Sign out</button></div></div>';
